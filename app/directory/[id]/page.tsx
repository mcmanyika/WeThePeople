...
import { getDirectoryListingById } from '@/lib/firebase/firestore'
...
export default function DirectoryDetailPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        setLoading(true)
        const data = await getDirectoryListingById(id)
        setListing(data)
      } finally {
        setLoading(false)
      }
  ...
  if (loading) {
    return <main className="min-h-screen bg-[#f8fbfa] p-8 text-center text-slate-500">Loading listing...</main>
  }
  ...
  if (!listing || listing.status !== 'approved') {
    return (
      <main className="min-h-screen bg-[#f8fbfa] px-4 py-16 text-center">
        ...
        <p className="mt-2 text-slate-600">This listing may be unavailable or unpublished.</p>
        <Link href="/directory" className="mt-6 inline-flex rounded-lg bg-emerald-500 px-5 py-2.5 text-sm font-semibold text-white hover:bg-emerald-600">
          Back to Directory
        </Link>
      </main>
    )
    ...
    <main className="min-h-screen bg-[#f8fbfa] text-slate-900">
      ...
      <section className="mx-auto max-w-5xl px-4 py-10 sm:px-6">
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
          ...
          <h1 className="text-3xl font-bold text-slate-900">{listing.businessName}</h1>
          <p className="mt-4 text-slate-700">{listing.shortDescription}</p>
          ...
          <div className="mt-6 grid gap-3 text-sm sm:grid-cols-2">
            ...
            {listing.website && (
              <p className="sm:col-span-2">
                ...
                <a href={listing.website} target="_blank" rel="noopener noreferrer" className="text-emerald-700 hover:underline break-all">
                  {listing.website}
                </a>
              </p>
            )}
